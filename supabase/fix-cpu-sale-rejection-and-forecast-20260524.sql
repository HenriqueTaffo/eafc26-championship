-- Corrige venda CPU recusada do Salah e evita que vendas CPU entrem na folha.
-- Aplicado em 24/05/2026.

update public.internal_transfer_proposals
   set status = 'rejected',
       response_message = 'Proposta da CPU recusada pelo vendedor. Corrigido administrativamente em 24/05/2026.',
       answered_by = coalesce(answered_by, seller),
       answered_at = coalesce(answered_at, now())
 where id = 7
   and lower(player) = lower('Mohamed Salah')
   and lower(seller) = lower('Renato')
   and lower(buyer) = lower('CPU');

update public.transfers
   set status = 'reversed',
       reason = 'Venda para CPU cancelada: proposta recusada pelo vendedor.',
       reversed_at = coalesce(reversed_at, now()),
       reversed_by = coalesce(reversed_by, 'Correcao administrativa'),
       updated_at = now()
 where id = 88
   and lower(player_name) = lower('Mohamed Salah')
   and transfer_type = 'cpu_sale';

update public.events
   set status = 'cancelled',
       description = 'Lancamento cancelado: proposta da CPU por Mohamed Salah foi recusada pelo vendedor.',
       effect = 'Sem impacto no orcamento.',
       updated_at = now()
 where id = 262
   and lower(title) = lower('Venda para CPU: Mohamed Salah');
