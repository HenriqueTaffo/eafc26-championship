-- Restrict the negotiable market catalog to players with Transfermarkt value
-- and a public salary reference (current or trusted historical).
...
create or replace view public.v_player_salary_reference_candidates as
  select
    ...
    public.app_salary_lookup_key(trim(r.player_name)) as player_lookup_key,
    public.app_salary_lookup_key(coalesce(trim(r.club_name), '')) as club_lookup_key,
  ...
  select
    ...
    public.app_salary_lookup_key(trim(coalesce(t.player_name, ''))) as player_lookup_key,
    public.app_salary_lookup_key(coalesce(trim(t.from_club), '')) as club_lookup_key,
  ...
  select
    ...
    t.id::bigint as candidate_id,
    public.app_salary_lookup_key(trim(coalesce(t.player_key, ''))) as player_lookup_key,
    public.app_salary_lookup_key(coalesce(trim(t.from_club), '')) as club_lookup_key,
    trim(coalesce(t.player_name, '')) as player_name,
    trim(coalesce(t.from_club, '')) as club_name,
    t.weekly_salary_eur,
  ...
  where public.app_public_salary_reference_is_valid(
    ...
    and public.app_salary_lookup_key(trim(coalesce(t.player_key, ''))) <>
      public.app_salary_lookup_key(trim(coalesce(t.player_name, '')));
...
create or replace view public.v_market_player_eligibility as
  with market_base as (
    select
      ...
      public.app_salary_lookup_key(coalesce(p.club, '')) as club_lookup_key,
      ...
      exists (
        ...
        where official.id <> p.id
          ...
          and public.app_salary_lookup_key(
...
update public.players_market p
   set name = fix.canonical_name,
       normalized_name = fix.canonical_normalized_name,
       transfermarkt_url = fix.transfermarkt_url
  from (
    values
      (1::bigint, 'Miguel Almiron', 'miguel almiron', 'https://www.transfermarkt.co.uk/miguel-almiron/profil/spieler/272999'),
      (7::bigint, 'Kevin De Bruyne', 'kevin de bruyne', 'https://www.transfermarkt.co.uk/kevin-de-bruyne/profil/spieler/88755'),
      (8::bigint, 'Angel Di Maria', 'angel di maria', 'https://www.transfermarkt.co.uk/angel-di-maria/profil/spieler/45320'),
      (9::bigint, 'Paulo Dybala', 'paulo dybala', 'https://www.transfermarkt.co.uk/paulo-dybala/profil/spieler/206050'),
      (14::bigint, 'Kepa Arrizabalaga', 'kepa arrizabalaga', 'https://www.transfermarkt.co.uk/kepa-arrizabalaga/profil/spieler/192279'),
      (15::bigint, 'Kalidou Koulibaly', 'kalidou koulibaly', 'https://www.transfermarkt.co.uk/kalidou-koulibaly/profil/spieler/93128'),
      (16::bigint, 'Luis Suarez', 'luis suarez', 'https://www.transfermarkt.co.uk/luis-suarez/profil/spieler/44352'),
      (20::bigint, 'Luka Modric', 'luka modric', 'https://www.transfermarkt.co.uk/luka-modric/profil/spieler/27992'),
      (22::bigint, 'Manuel Neuer', 'manuel neuer', 'https://www.transfermarkt.co.uk/manuel-neuer/profil/spieler/17259')
  ) as fix(id, canonical_name, canonical_normalized_name, transfermarkt_url)
 where p.id = fix.id;
