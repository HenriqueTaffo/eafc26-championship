...
App.governance = {
  async loadData() {
    try {
      ...
      App.state.apiGovernance = App.state.apiGovernance || {
        auctions: [],
        medicalActions: [],
        weeklyReviews: [],
      };
  ...
  getMarketPhase() {
    ...
    if (hour < 9)
      return {
        ...
        detail: "Apenas propostas internas e planejamento.",
        tone: "closed",
      };
    if (hour < 13)
      return {
        name: "Mercado calmo",
        detail: "Bom momento para consultas e ajustes finos.",
        tone: "calm",
      };
    if (hour < 18)
      return {
        name: "Janela ativa",
        detail: "Compras, leilões e negociações liberadas.",
        tone: "active",
      };
    if (hour < 21)
      return {
        name: "Deadline day",
        detail: "Negociações pesadas entram no radar de leilão.",
        tone: "deadline",
      };
    return {
      name: "Mercado fechado",
      detail: "Use a noite para revisar orçamento e propostas.",
      tone: "closed",
    };
  },
  ...
  getRivalries() {
    const standings = App.standings
      .getStandings()
      .filter((row) => row.status === "Nosso");
    return standings.map((team) => {
      const nearest = standings
        .filter((other) => other.owner !== team.owner)
        .sort(
          (a, b) =>
            Math.abs((a.points || 0) - (team.points || 0)) -
            Math.abs((b.points || 0) - (team.points || 0)),
        )[0];
  ...
  getReputationRows() {
    ...
    return App.utils.getHumanBuyers().map((owner) => {
      const bought = transfers.filter(
        (item) =>
          App.utils.normalizeText(item.buyer) ===
      ...
      const sold = proposals.filter(
        (item) =>
          App.utils.normalizeText(item.seller) ===
            App.utils.normalizeText(owner) && item.status === "accepted",
      );
  ...
  getEconomyRows() {
    if (
      Array.isArray(App.state.apiFinanceForecast) &&
      App.state.apiFinanceForecast.length
    ) {
      return App.state.apiFinanceForecast
        .map((item) => ({
          ...
          runwayWeeks:
            item.runway_weeks === null ? null : Number(item.runway_weeks),
          ...
          burnRate:
            Number(item.total_budget || 0) > 0
              ? (Number(item.spent_total || 0) +
                  Number(item.payroll_monthly || 0)) /
                Number(item.total_budget || 1)
              : 0,
        }))
    ...
    return App.transfers
      ...
      .map((item) => {
        ...
        const burnRate =
          item.totalBudget > 0
            ? (item.spent + projectedMonth) / item.totalBudget
            : 0;
        const risk =
          item.remaining < 0
            ? "Crítico"
            : burnRate >= 0.95
              ...
              : burnRate >= 0.78
                ? "Atenção"
                : item.payrollWeekly > 0 &&
                    item.runwayWeeks !== null &&
                    item.runwayWeeks < 4
                  ? "Folha curta"
                  : "Saudável";
        ...
        return {
          ...
          marketEmbargo: Boolean(
            item.marketEmbargo || item.salaryDebtActive || item.remaining < 0,
          ),
          salaryDebtActive: Boolean(item.salaryDebtActive),
          salaryDebtAmount: Number(
            item.salaryDebtAmount ||
              (item.remaining < 0 ? Math.abs(item.remaining) : 0),
          ),
  ...
  getPublicRumors() {
    ...
    transfers
      .filter(
        (item) =>
          Number(item.totalCost || 0) >= 22000000 ||
          Number(item.overall || 0) >= 84,
      )
      .slice(-5)
      .reverse()
      .forEach((item) =>
    ...
    fairPlay.slice(0, 3).forEach((item) =>
      rows.push({
  ...
  getWeeklyObjectiveRows() {
    ...
    return App.data.teams
      ...
      .map((team) => {
        const standing = standings.find((row) =>
          App.utils.sameTeamName(row.team, team.team),
        );
        ...
        const managerId = App.utils
          .normalizeText(team.owner)
          .replace(/\s+/g, "-");
        return {
          managerId,
          owner: team.owner,
  ...
  getEventKey(event = {}) {
    return [
      ...
      App.cups?.normalizeCupPhase
        ? App.cups.normalizeCupPhase(event.phase || "")
  ...
  findDbMatchForEvent(event, dbEvents = []) {
    return (
      dbEvents.find(
        (dbEvent) =>
          App.governance.isSameCompetition(
            event.competition,
            dbEvent.competition,
          ) &&
          (App.utils.normalizeText(event.phase) ===
            App.utils.normalizeText(dbEvent.phase) ||
            App.cups?.cupPhasesAreCompatible?.(event.phase, dbEvent.phase)) &&
          App.cups?.sameCupTeams?.(
            event.home,
            event.away,
            dbEvent.home,
            dbEvent.away,
  ...
  getIntegrityAudit() {
    ...
    const resultGroups = approvedResults
      ...
      .reduce((acc, row) => {
        const key = [
          ...
          App.cups?.normalizeCupPhase
            ? App.cups.normalizeCupPhase(row.RodadaFase || "")
            : App.utils.normalizeText(row.RodadaFase),
          [
            App.utils.normalizeTeamName(row.Mandante),
            App.utils.normalizeTeamName(row.Visitante),
          ]
            .sort()
            .join("~"),
        ].join("|");
    ...
    App.transfers.getFairPlayWatchlist().forEach((item) => {
      issues.push({
        severity:
          item.marketEmbargo || item.remaining < 0 ? "critical" : "warn",
    ...
    return {
      ...
      score: Math.max(
        0,
        100 -
          issues.filter((item) => item.severity === "critical").length * 25 -
          issues.filter((item) => item.severity === "warn").length * 10,
      ),
    };
  },
    ...
    return [
      ...
      ...auctions
        ...
        .map((item) => ({
          tone: "deadline",
          title: "Leilão aberto",
          detail: `${item.player_name} · ${App.utils.formatCurrency(item.opening_value)}`,
        })),
      ...(injuries.length
        ? [
            {
              tone: "warn",
              title: "DM movimentado",
              detail: `${injuries.length} lesão(ões) ativa(s) na liga.`,
            },
          ]
        : []),
      ...(targets.length
        ? [
            {
              tone: "calm",
              title: "Shortlist privada",
              detail: `${targets.length} alvo(s) no seu radar privado.`,
            },
          ]
        : []),
    ];
  ...
  async runAction(action, payload = {}) {
    ...
    if (!session)
      throw new Error(
        "Faça login como técnico antes de usar a mesa do comissário.",
      );
    if (!App.auth?.isCommissioner?.())
      throw new Error("Apenas o Comissário da Liga pode executar esta ação.");
    ...
    if (result?.ok === false)
      throw new Error(result.message || "Ação recusada pelo Supabase.");

    await App.api.loadApiData({
      variant: "chaos",
      title: "Atualizando governança",
      message:
        "Aplicando ação especial, sincronizando eventos, mercado e painéis.",
    });
      ...
      <article class="commissioner-card commissioner-integrity-card" id="commissionerIntegrity">
        ...
        <div class="commissioner-list integrity-list">
          ...
          `,
                  ...
                  .join("")
              : `<p class="calendar-muted">Nenhuma inconsistência crítica detectada.</p>`
          }
        </div>
        ${
  ...
  renderTransferReversal() {
    ...
    const rows = (App.state.apiTransfers || [])
      .filter((item) =>
        App.transfers.isApprovedTransferStatus(item.Status || item.status),
      )
      .slice()
      .sort((a, b) => {
    ...
    const groupNames = [
      ...
      ...Object.keys(groupedRows)
        .filter((owner) => !ownerOrder.includes(owner))
        .sort(),
    ];
    ...
    return `
      <article class="commissioner-card commissioner-transfer-reversal-card">
                ...
                <optgroup label="${App.utils.escapeHtml(groupName)}">
                  ${groupedRows[groupName]
                    .map((item, index) => {
                      ...
                      const value = Number(
                        item.ValorFinal || item.ValorTransfermarkt || 0,
                      );
                      return `<option
        ...
        `
            : rows.length
              ? `<p class="calendar-muted">Faça login como Comissário da Liga para desfazer transferências.</p>`
              : `<p class="calendar-muted">Nenhuma transferência aprovada para desfazer.</p>`
        }
      </article>
    `;
  },
      ...
      <article class="commissioner-card" id="commissionerAuctions">
        ...
        ${
          ...
          <button class="secondary-button success" type="submit">Abrir leilão</button>
            ...
            : `<p class="calendar-muted">Faça login como Comissário da Liga para abrir leilões.</p>`
        }
        <div class="commissioner-list">
          ...
          `,
                  ...
                  .join("")
              : `<p class="calendar-muted">Nenhum leilão registrado ainda.</p>`
          }
        </div>
        <div class="commissioner-sublist">
      ...
      <article class="commissioner-card" id="commissionerMedical">
        ...
        <div class="commissioner-list medical-list">
          ...
          `,
                  ...
                  .join("")
              : `<p class="calendar-muted">Nenhuma lesão ativa para tratar.</p>`
          }
        </div>
          ...
          ${
            reviews.length
              ? reviews
                  ...
                  .map(
                    (item) =>
                      `<span>${App.utils.escapeHtml(item.manager_name)} · ${item.objectives_met}/${item.objectives_total} · ${App.utils.escapeHtml(item.verdict)}</span>`,
                  )
                  .join("")
              : `<span>Nenhum fechamento registrado.</span>`
          }
        </div>
      </article>
    `;
  ...
  bindActions(root = document) {
    ...
    const clearButton = root.querySelector(
      "[data-commissioner-clear-injuries]",
    );
      ...
      weeklyButton.addEventListener("click", async () => {
        try {
          const result = await App.governance.runAction(
            "app_close_weekly_review",
            {
              p_snapshot: JSON.stringify(
                App.governance.getWeeklyObjectiveRows(),
              ),
            },
          );
          ...
          App.utils.setMessage(
            message,
            `Fechamento semanal registrado.${payrollMessage}`,
            "success",
          );
        } catch (error) {
          App.utils.setMessage(message, error.message, "error");
    ...
    root.querySelectorAll("[data-reverse-transfer-form]").forEach((form) => {
      ...
      form.addEventListener("submit", async (event) => {
        ...
        if (
          !window.confirm(
            `Desfazer a transferência de ${player} para ${buyer}?`,
          )
        )
          return;
        ...
        try {
          ...
          if (result?.ok === false)
            throw new Error(
              result.message || "Transferência não foi desfeita.",
            );
          await App.api.loadApiData({
            variant: "market",
            title: "Atualizando mercado",
            message:
              "Transferência desfeita. Recalculando orçamento, elenco e radar.",
          });
          App.utils.setMessage(
            message,
            result.message || "Transferência desfeita.",
            "success",
          );
    ...
    root.querySelectorAll("[data-cpu-transfer-offers-form]").forEach((form) => {
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          App.utils.setMessage(
            message,
            result.message || "Propostas externas geradas.",
            "success",
          );
        } catch (error) {
          App.utils.setMessage(message, error.message, "error");
